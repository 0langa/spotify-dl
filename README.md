...
- Scheduled auto-sync: sources marked Keep in sync are checked on a chosen interval and new tracks are downloaded through the queue
...
- Auto-sync runs only while Playlist DL is open. It is skipped while anything else is running and resumes at the next check, so an interval is an upper bound on how often a source is checked, not a guarantee.
