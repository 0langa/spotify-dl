...
- When the output folder cannot be read in full, the check reports what it saw, relocates nothing, and does not offer to reopen tracks, because an unread file is indistinguishable from a deleted one. A folder that is not available at all (unplugged drive, offline share, renamed parent) is reported as unavailable rather than as deleted files.
- Paths are compared in canonical form. A file reached through a junction or a symlink under a different path is treated as a different file, so it is neither adopted nor recognized as claimed.
